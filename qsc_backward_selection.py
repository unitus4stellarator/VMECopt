...
def create_folders(vmec_files_dir,date,time):
	...
	if not os.path.exists(path+'/vmec_files_points'):
		os.mkdir(path+'/vmec_files_points')

	vmec_files_points_path=path+'/vmec_files_points/'+date.strftime('%d%m%Y')+'_'+time.strftime('%H%M%S')
	
	os.mkdir(vmec_files_points_path)
	os.chdir(vmec_files_points_path)
	
	readme(vmec_files_path)
	
	return vmec_files_path,threed1_files_path,vmec_files_patched_path,vmec_files_graphs_path,vmec_files_points_path
...
def hist3d(x,y,z_size,z_max,title,zlabel,filename):
	fig = plt.figure()
...
vmec_files_dir='/vmec_files/14062022_142054'

vmec_files=get_vmec_files(vmec_files_dir)

[vmec_files_path,threed1_files_path,vmec_files_patched_path,vmec_files_graphs_path,vmec_files_points_path]=create_folders(vmec_files_dir,date.today(),datetime.now())
...
for vmec_file in vmec_files:
	...
	if vmec is not None:
		...
		pandas.DataFrame(gamma.transpose(2,0,1).reshape(-1,gamma.shape[2])).to_csv(vmec_file+'.csv', index = False, header  = ['X','Y','Z'], sep=';')  
		
		#numpy.savetxt(vmec_file+'.csv', gamma.transpose(2,0,1).reshape(-1,gamma.shape[2]), fmt = '%f', delimiter=",")
		...
		hist3d(R[:, :, 0].reshape((mpol)*(2*ntor+1)),R[:, :, 1].reshape((mpol)*(2*ntor+1)),R[:, :, 2].reshape((mpol)*(2*ntor+1)),0.001,
		vmec_file+'\n$|R_{m,n}(\\theta,\phi)|$','$|R_{m,n}|$',vmec_file+'_R_zoom.png')
		
		hist3d(Z[:, :, 0].reshape((mpol)*(2*ntor+1)),Z[:, :, 1].reshape((mpol)*(2*ntor+1)),Z[:, :, 2].reshape((mpol)*(2*ntor+1)),0.001,
		vmec_file+'\n$|Z_{m,n}(\\theta,\phi)|$','$|Z_{m,n}|$',vmec_file+'_Z_zoom.png')

		hist3d(R[:, :, 0].reshape((mpol)*(2*ntor+1)),R[:, :, 1].reshape((mpol)*(2*ntor+1)),R[:, :, 2].reshape((mpol)*(2*ntor+1)),1,
		vmec_file+'\n$|R_{m,n}(\\theta,\phi)|$','$|R_{m,n}|$',vmec_file+'_R.png')
		
		hist3d(Z[:, :, 0].reshape((mpol)*(2*ntor+1)),Z[:, :, 1].reshape((mpol)*(2*ntor+1)),Z[:, :, 2].reshape((mpol)*(2*ntor+1)),1,
		...
		while mean_error<min_mean_error and index_out_of_range==False:
			...
			if mean_error<min_mean_error and index_out_of_range==False:
				...
				if trim_m<mpol:
					...
					pandas.DataFrame(gamma_patched.transpose(2,0,1).reshape(-1,gamma_patched.shape[2])).to_csv(vmec_file+'_patched.csv', index = False, header  = ['X','Y','Z'], sep=';')  
					
					#numpy.savetxt(vmec_file+'_patched.csv', gamma_patched.transpose(2,0,1).reshape(-1,gamma_patched.shape[2]), fmt = '%f', delimiter=",")
					...
					hist3d(R_patched[:, :, 0].reshape((mpol)*(2*ntor+1)),R_patched[:, :, 1].reshape((mpol)*(2*ntor+1)),R_patched[:, :, 2].reshape((mpol)*(2*ntor+1)),0.001,
					vmec_file+'_patched\n$|R_{m,n}(\\theta,\phi)|$','$|R_{m,n}|$',vmec_file+'_patched_R_zoom.png')
					
					hist3d(Z_patched[:, :, 0].reshape((mpol)*(2*ntor+1)),Z_patched[:, :, 1].reshape((mpol)*(2*ntor+1)),Z_patched[:, :, 2].reshape((mpol)*(2*ntor+1)),0.001,
					vmec_file+'_patched\n$|Z_{m,n}(\\theta,\phi)|$','$|Z_{m,n}|$',vmec_file+'_patched_Z_zoom.png')

					hist3d(R_patched[:, :, 0].reshape((mpol)*(2*ntor+1)),R_patched[:, :, 1].reshape((mpol)*(2*ntor+1)),R_patched[:, :, 2].reshape((mpol)*(2*ntor+1)),1,
					vmec_file+'_patched\n$|R_{m,n}(\\theta,\phi)|$','$|R_{m,n}|$',vmec_file+'_patched_R.png')
					
					hist3d(Z_patched[:, :, 0].reshape((mpol)*(2*ntor+1)),Z_patched[:, :, 1].reshape((mpol)*(2*ntor+1)),Z_patched[:, :, 2].reshape((mpol)*(2*ntor+1)),1,
